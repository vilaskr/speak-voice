...
export const CheckStatusForm: React.FC<CheckStatusFormProps> = ({ initialReportId = '', onGoBackToSubmit }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4 sm:px-6 flex-grow">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 font-mono text-base font-bold tracking-wider uppercase focus:ring-2 focus:ring-indigo-500 outline-none text-slate-900"
              />
              <button
                id="check-status-btn"
                type="submit"
                disabled={isLoading}
                className="bg-indigo-900 hover:bg-black disabled:bg-indigo-400 text-white font-bold px-6 py-3 rounded-lg transition-colors shadow-sm shrink-0 cursor-pointer flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
        ...
        {/* Error State */}
        {errorMessage && (
          <div className="flex items-start space-x-3 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-900 text-sm animate-in fade-in">
        ...
        {statusResult && (
          <div className="pt-4 border-t border-slate-200 space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 rounded-xl bg-slate-50 border border-slate-200">
              <div>
                ...
                <div className="mt-1 text-xs text-slate-500">
                  Category: <span className="font-medium text-slate-800">{statusResult.category}</span>
                </div>
              </div>
            ...
            {/* Timestamps */}
            <div className="grid grid-cols-2 gap-4 pt-3 text-xs text-slate-500 border-t border-slate-100">
              <div>
            ...
            {/* Privacy note */}
            <div className="p-3.5 rounded-xl bg-slate-100 text-slate-600 text-xs flex items-center space-x-2">
      ...
      {onGoBackToSubmit && (
        <div className="mt-6 text-center">
          <button
            ...
            className="inline-flex items-center space-x-1.5 text-xs font-semibold text-slate-600 hover:text-indigo-600 transition-colors cursor-pointer"
          >
